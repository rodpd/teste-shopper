  ...
  async create(req, res) {
    ...
    const measure_mes = await Measure.findOne(
      {
        where: {
          measure_datetime: {
            [Op.like]: measure_datetime.slice(0,7)
          }
        }
      }
    )
    ...
    if (measure_mes) {
      ...
      try {
        ...
        const response = await Measure.create({measure_datetime: measure_datetime.slice(0,7), measure_type: measure_type, measure_value: parseInt(result.response.text()), image_url: fileUri, CustomerId: customer_code  })
